...
export const Courses = () => {
	...
	const firstCourse = isFirstPage
  ? 0
  : totalFirstPage + (page - 2) * 6;

	const lastCourse = isFirstPage
		? totalFirstPage
		: firstCourse + 6;
	...
	return (
		<section className={cl.courses}>
			...
			<div className={cl.courses__items}>
				{page !== 1 ? (
					<div className={`${cl.courses__paginate} ${cl.prev__course}`} onClick={() => setPage(prev => Math.max(prev - 1, 1))}>
				...
				{lastPage > page ? (
					<div className={`${cl.courses__paginate} ${cl.next__course}`} onClick = {() => setPage(prev => Math.min(prev + 1, lastPage))}>
